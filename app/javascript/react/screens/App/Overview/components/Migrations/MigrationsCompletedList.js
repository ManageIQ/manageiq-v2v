        ...
        <ListView className="plans-complete-list" style={{ marginTop: 0 }}>
          {finishedTransformationPlans.map(plan => {
            ...
            if (plan.options.config_info.vm_ids.length > 1) {
              ...
              const planVMTasks = flattenAllTasks.filter(task => plan.options.config_info.vm_ids.indexOf(task.source_id) > -1);
              ...
              vmTasksForRequestOfPlan.push(plan.options.config_info.vm_ids.map(vmId => getMostRecentTaskFromGroup(groupedByVMId[vmId])));

              const flattenVMTasksForRequestOfPlan = [];
              vmTasksForRequestOfPlan.map(task => task.map(arrayElement => flattenVMTasksForRequestOfPlan.push(arrayElement)));
